...
final class CommandDispatcher
{
    ...
    private function dispatchInScope(array $argv, ExecutionScope $rootScope, ?SignalState $signals = null): int
    {
        ...
        if (!$rootScope instanceof ScopeIdentity) {
            throw new RuntimeException('Console command scopes require Runtime scope identity.');
        }
    ...
    private function execute(
        ...
        if ($command === 'help') {
            $lifecycle->activate('console.help');
            $this->writeHelp($args);
        ...
        if ($helpTarget !== null) {
            ...
            $lifecycle->activate("console.group.$name.help");
        ...
        if ($commandHelpPath !== null) {
            ...
            $lifecycle->activate("console.command.$name.help");
        ...
        if (!in_array($command, $this->commands->keys(), true)) {
            $lifecycle->activate('console.unknown');

            throw UnknownCommand::named($command);
        }

        $lifecycle->activate("console.command.$command");
